...
with open(stats_file_name, "w") as f:
    ...
    files_kw_percentage = extractor.kw_ratio_dataframe(df_projects, "files_with_kw", "files")
    ...
    f.write(f"The number of projects: {extractor.num_after_cleanup(df_before, df_projects)}, that are no longer reachable\n")
    f.write(f"The percentage: {100 - extractor.percentage_after_cleanup(df_before, df_projects)}%, that are no longer reachable\n")
    ...
    for kw in file_names:
        ...
        files_kw_percentage = extractor.kw_ratio_dataframe(df_projects, "files_with_" + path, "files")
        ...
        f.write(f"The maximum number of occurrences for {extractor.file_to_kw.get(kw)} in a project is: {max_kw_project}\n")

        min_kw_project = extractor.min_keyword_project(df_projects, path)
        f.write(f"The minimum number of occurrences for {extractor.file_to_kw.get(kw)} in a project is: {min_kw_project}\n")
